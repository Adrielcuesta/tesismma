...
def intentar_parsear_json_riesgos(texto_llm_str):
    if not isinstance(texto_llm_str, str):
        logger.error("Error de parseo: la entrada para `intentar_parsear_json_riesgos` no es un string.") # CAMBIO
        return None
    ...
    if not (json_candidate_str.startswith("{") and json_candidate_str.endswith("}")):
        ...
        if first_brace != -1 and last_brace != -1 and last_brace > first_brace:
            ...
            logger.warning("Advertencia de parseo: No se encontró una estructura JSON clara ({...}) en la respuesta del LLM.") # CAMBIO
            return None
    ...
    try:
        ...
        logger.error(f"Error de parseo JSON: {e}") # CAMBIO
        logger.debug(f"Texto que intentó parsearse (después de limpieza): '{json_candidate_str[:500]}...'") # CAMBIO
        return None
    except Exception as e_gen:
        logger.error(f"Error de parseo inesperado: {e_gen}") # CAMBIO
...
def formatear_y_guardar_reporte(resultado_analisis_llm, fuentes_recuperadas, 
    ...
    logger.info("Procesando salida del LLM para formato estructurado (JSON)...") # CAMBIO
    ...
    reporte_final = {
        ...
        "fuentes_contexto_recuperadas": fuentes_recuperadas if fuentes_recuperadas else [] # Asegurar que sea una lista
    }

    # La lógica de extraer fuentes ya está en main.py y se pasa 'fuentes_recuperadas_serializables'
    ...
    if isinstance(resultado_analisis_llm, str):
        ...
        if json_output_llm and "riesgos_identificados" in json_output_llm and \
           isinstance(json_output_llm["riesgos_identificados"], list):
            logger.info("JSON de riesgos parseado exitosamente desde la respuesta del LLM.") # CAMBIO
            for riesgo_item_llm in json_output_llm["riesgos_identificados"]:
                if isinstance(riesgo_item_llm, dict):
                    # ... (lógica de parseo de riesgo individual, ya parece correcta) ...
                    ...
                    logger.warning(f"Item de riesgo no es un diccionario: {riesgo_item_llm}") # CAMBIO
                    # ... (manejo de error de item) ...
            # ... (logs de advertencia si la lista está vacía o no se procesó) ...
        else:
            logger.warning("No se pudo parsear un JSON estructurado de riesgos desde la respuesta del LLM.") # CAMBIO
    else:
        logger.warning("El resultado del análisis no es un string, no se intentará parseo JSON.") # CAMBIO

    timestamp_file = datetime.now().strftime("%Y%m%d_%H%M%S")
    clean_project_name = "".join(c if c.isalnum() else "_" for c in nombre_pdf_proyecto.replace('.pdf',''))
    output_filename_json = f"analisis_riesgos_{clean_project_name}_{timestamp_file}.json" # Ajustado
    ...
    try:
        ...
        logger.info(f"Reporte JSON guardado en: {output_file_path_json}") # CAMBIO
        return output_file_path_json
    except Exception as e_save:
        logger.error(f"Error crítico al guardar el archivo JSON de resultados: {e_save}") # CAMBIO
        logger.debug(traceback.format_exc())
        return None
