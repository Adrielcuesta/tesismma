...
try:
    from .config import PROJECT_ROOT, MODELOS_LOCALES_PATH, CACHE_DIR_HF, DEFAULT_EMBEDDING_MODEL_HF_REPO_ID
except ImportError:
...
def descargar_modelo():
    logger.info(f"Verificando/creando directorio base de modelos locales: {MODELOS_LOCALES_PATH}")
    if not os.path.exists(MODELOS_LOCALES_PATH):
        try:
    ...
    logger.info(f"Verificando/creando directorio de guardado específico del modelo: {FULL_SAVE_PATH}")
    ...
    try:
        ...
        if 'original_requests_ca_bundle' in locals():
            if original_requests_ca_bundle is not None: os.environ['REQUESTS_CA_BUNDLE'] = original_requests_ca_bundle
            elif 'REQUESTS_CA_BUNDLE' in os.environ: del os.environ['REQUESTS_CA_BUNDLE']
